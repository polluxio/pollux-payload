...
def run(partitions, commandLine):
  dirName = 'eschetest'
  #create eschertest dir, erase it first if it exists
  if os.path.exists(dirName):
    shutil.rmtree(dirName)
  os.mkdir(dirName)

  topYamlName = 'escher.yaml'
...
def main() -> int:
  parser = ArgumentParser(prog='escherapptest', exit_on_error=False)
  ...
  logging.info("escherapptest terminated")
  return 0
